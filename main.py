...
def add_soft_sum_hours_constraint(
    ...
    if soft_min > hard_min and min_cost > 0:
        delta = model.NewIntVar(-len(works), len(works), "")
        model.Add(delta == soft_min - sum_var)
        excess = model.NewIntVar(0, 640, prefix + ": under_sum")
        model.AddMaxEquality(excess, [delta, 0])
        cost_variables.append(excess)
        cost_coefficients.append(min_cost)
    ...
    # Penalize sums above the soft_max target.
    if soft_max < hard_max and max_cost > 0:
        delta = model.NewIntVar(-640, 640, "")
        model.Add(delta == sum_var - soft_max)
        print(sum_var - soft_max)
        excess = model.NewIntVar(0, 640, prefix + ": over_sum")
        model.AddMaxEquality(excess, [delta, 0])
        cost_variables.append(excess)
        cost_coefficients.append(max_cost)
...
def solve_shift_scheduling(params, output_proto):
    ...
    shift_constraints = [
        # One to five consecutive days of rest, this is a hard constraint.
        (0, 1, 1, 0, 5, 6, 1),
        # between 2 and 3 consecutive days of night shifts, 1 and 4 are
        # possible but penalized.
        (3, 2, 2, 1, 4, 4, 1),
        # (3, 1, 2, 20, 3, 4, 5), <- previous constraint
        # between
    ]
    ...
    monthly_sum_constraints = [
        # At least 0 night shift per month (penalized). At most 4 (hard).
        (3, 0, 1, 3, 3, 4, 0),
    ]

    # Monthly sum constraints on weekends:
    #     (shift, hard_min, soft_min, min_penalty,
    #             soft_max, hard_max, max_penalty)
    monthly_weekend_sum_constraints = [
        # at least 2 weekend off per month
        (0, 4, 4, 1, 8, 8, 1),
    ]
    ...
    penalized_transitions = [
        ...
        (3, 1, 0),
        # Night to evening is forbidden.
        (3, 2, 0),
        # Evening to morning is forbidden.
        (2, 1, 0),
    ]

    # Weekend constraints:
    #       (previous_shift, next_shift, penalty (0 means ignored))
    weekend_constraints = [
        (3, 0, 50),
        (2, 0, 50),
        (2, 3, 50),
    ...
    # After a night shift, employee must have three days off
    #     (previous_shift, next_shift, penalty (0 means forbidden))
    night_transitions = [
        # Night to next three days is forbidden.
        (3, 1, 0),
        (3, 2, 0),
    ]
    ...
    for ct in monthly_sum_constraints:
        ...
        for e in range(num_employees):
            for m in range(num_months):
                works = [work[e, shift, d + m * 28] for d in range(28)]
                variables, coeffs = add_soft_sum_constraint(
                    model,
                    works,
                    hard_min,
                    ...
                    hard_max,
                    max_cost,
                    "monthly_sum_constraints(employee %i, shift %i, month %i)"
                    % (e, shift, m),
                )
                obj_int_vars.extend(variables)
                obj_int_coeffs.extend(coeffs)

    # Monthly weekend sum constraints
    for ct in monthly_weekend_sum_constraints:
        shift, hard_min, soft_min, min_cost, soft_max, hard_max, max_cost = ct
        for e in range(num_employees):
            works = [work[e, shift, d] for d in weekends]
            variables, coeffs = add_soft_sum_constraint(
                model,
                works,
                hard_min,
    ...
    # Weekend equal shift constraints
    for e in range(num_employees):
        for w in range(num_weeks):
            for previous_shift, next_shift, cost in weekend_constraints:
                transition = [
                    work[e, previous_shift, 5 + w * 7].Not(),
                    work[e, next_shift, 6 + w * 7].Not(),
                ]
                if cost == 0:
                    model.AddBoolOr(transition)
                else:
                    trans_var = model.NewBoolVar(
                        "transition in weekend (employee=%i, day=%i)" % (e, 5 + w * 7)
                    )
                    transition.append(trans_var)
                    model.AddBoolOr(transition)
                    obj_bool_vars.append(trans_var)
    ...
    # Night transitions
    for previous_shift, next_shift, cost in night_transitions:
        for e in range(num_employees):
            for d in range(num_days - 2):
                transition = [
                    work[e, previous_shift, d].Not(),
                    work[e, next_shift, d + 2].Not(),
                ]
                if cost == 0:
                    model.AddBoolOr(transition)
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        #             for e in range(num_employees):
        #                 if solver.BooleanValue(work[e, s, d]):
        #                     print("    employee %i works on shift %s" % (e, shifts[s]))
        # print()

        # Calculate and print an overview of how many N shifts each employee has worked in total.
        for e in range(num_employees):
            for s in range(num_shifts):
                if shifts[s] == "N":
